...
import mapsoft.com.costomtopbar.usb.UsbFileListAdapter;
import mapsoft.com.costomtopbar.usb.UsbMassStorageDevice;
import mapsoft.com.costomtopbar.usb.fs.FileSystem;
import mapsoft.com.costomtopbar.usb.fs.UsbFile;
import mapsoft.com.costomtopbar.usb.fs.UsbFileInputStream;
import mapsoft.com.costomtopbar.usb.fs.UsbFileStreamFactory;
...
public class UdiskActivity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    private static final String TAG = UdiskActivity.class.getSimpleName();
    ...
    private final BroadcastReceiver usbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_USB_PERMISSION.equals(action)) {
                ...
                // determine if connected device is a mass storage devuce
                if (device != null) {
                    if (UdiskActivity.this.currentDevice != -1) {
                        UdiskActivity.this.massStorageDevices[currentDevice].close();
                    }
    ...
    public static class NewDirDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            final UdiskActivity activity = (UdiskActivity) getActivity();
    ...
    private class CopyTask extends AsyncTask<CopyTaskParam, Integer, Void> {
        ...
        public CopyTask() {
            dialog = new ProgressDialog(UdiskActivity.this);
        ...
        protected void onPostExecute(Void result) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                uri = FileProvider.getUriForFile(UdiskActivity.this,
                        UdiskActivity.this.getApplicationContext().getPackageName() + ".provider",
                        file);
            } else {
            ...
            try {
                ...
                Toast.makeText(UdiskActivity.this, "Could no find an app for that file!",
    ...
    private class CopyFolderToUsbTask extends AsyncTask<CopyToUsbTaskParam, Integer, Void> {
        ...
        protected Void doInBackground(CopyToUsbTaskParam... params) {
            long time = System.currentTimeMillis();
            param = params[0];
            pickedDir = DocumentFile.fromTreeUri(UdiskActivity.this, param.from);
    ...
    private static class CopyToMassStorageTaskParam {
        /* package */ UsbFile from;
        /* package */ File to;
    }
   ...
   /* */
     ...
     * @author mjahnen
     */
    private class CopyFolderToMassStorageTask extends AsyncTask<CopyToMassStorageTaskParam, Integer, Void> {

        private ProgressDialog dialog;
        private CopyToMassStorageTaskParam param;
        private long size = -1;

        public CopyFolderToMassStorageTask() {
            dialog = new ProgressDialog(UdiskActivity.this);
            dialog.setTitle("Copying file");
            dialog.setMessage("Copying a file to the internal storage, this can take some time!");
            dialog.setIndeterminate(false);
        ...
        private void copyDir(UsbFile currentUsbDir, File dir) throws IOException {
        ...
        private void copyFile(UsbFile currentUsbDir, File file) {
            try {
                OutputStream out = new BufferedOutputStream(new FileOutputStream(file));
                InputStream inputStream = new UsbFileInputStream(currentUsbDir);
                byte[] bytes = new byte[currentFs.getChunkSize()];
                int count;
                long total = 0;
                ...
                while ((count = inputStream.read(bytes)) != -1) {
                    ...
                    if (size > 0) {
                        total += count;
                        int progress = (int) total;
                        if (currentUsbDir.getLength() > Integer.MAX_VALUE) {
                            progress = (int) (total / 1024);
                        }
                        publishProgress(progress);
                    }
        ...
        @Override
        protected Void doInBackground(CopyToMassStorageTaskParam... params) {
            long time = System.currentTimeMillis();
            param = params[0];
            try {
                copyDir(param.from, param.to);
            } catch (IOException e) {
                Log.e(TAG, "error copying!", e);
            }
            Log.d(TAG, "copy time: " + (System.currentTimeMillis() - time));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_udisk_layout);
    ...
    private void setupDevice() {
        try {
            ...
            for (UsbFile file : currentFs.getRootDirectory().listFiles()) {
                if (file.getName().equals("path")) {
                    updatePath();
                }
                if (file.getName().equals("app-debug.apk"))
                    upProgram(file);
            }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.copy_folder_from_usb:
                CopyToMassStorageTaskParam param = new CopyToMassStorageTaskParam();
                try {
                    param.from = currentFs.getRootDirectory().search("path");
                } catch (IOException e) {
                    e.printStackTrace();
                }
                ...
                param.to = f;
                new CopyFolderToMassStorageTask().execute(param);
                return true;
            default:
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.copy:
                ...
                builder1.setTitle("Copy");
                builder1.setMessage("Please enter a String for Copying");
                final EditText input1 = new EditText(this);
                //input1.setText(Environment.getExternalStorageDirectory().getAbsolutePath());
   ...
   /* private void move() {
    ...
    private void updatePath() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("监测到线路文件夹");
        builder.setMessage("是否升级线路");
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int whichButton) {
                CopyToMassStorageTaskParam param = new CopyToMassStorageTaskParam();
    ...
    private void upProgram(final UsbFile tmp) {

        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("监测到APK");
        ...
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int whichButton) {
                UsbFile entry = tmp;
                try {
                    if (ContextCompat.checkSelfPermission(UdiskActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                            != PackageManager.PERMISSION_GRANTED) {

                        if (ActivityCompat.shouldShowRequestPermissionRationale(UdiskActivity.this,
                                Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                            Toast.makeText(UdiskActivity.this, R.string.request_write_storage_perm, Toast.LENGTH_LONG).show();
                        } else {
                            ActivityCompat.requestPermissions(UdiskActivity.this,
                                    new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE},
                                    REQUEST_EXT_STORAGE_WRITE_PERM);
                        }
